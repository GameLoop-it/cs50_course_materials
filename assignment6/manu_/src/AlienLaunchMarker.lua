...
function AlienLaunchMarker:splitPlayer()
    local player = self.aliens[1]
    ...
    --first alien ( upper copy )
    local alien = Alien(self.world, 'round', x, y-16, 'Player')
    alien.body:setLinearVelocity(dx, dy - 16)
    alien.fixture:setRestitution(0.4)
    alien.body:setAngularDamping(1)
    table.insert(self.aliens, alien)

    --third alien ( lower copy )
    local alien = Alien(self.world, 'round', x, y+16, 'Player')
    alien.body:setLinearVelocity(dx, dy + 16)
    alien.fixture:setRestitution(0.4)
    alien.body:setAngularDamping(1)
