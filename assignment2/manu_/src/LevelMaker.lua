...
function LevelMaker.createMap(level, containsLockedBrick)
    local bricks = {}
    ...
    if containsLockedBrick then
        ...
        bricks[lockedBrickIndex] = LockedBrick(bricks[lockedBrickIndex].x, bricks[lockedBrickIndex].y)    
    end
...
--Thanks to @Karbb
function LevelMaker.createTestMap(level, containsLockedBrick)
    local bricks = {}
    ...
    -- in the event we didn't generate any bricks, try again
    if #bricks == 0 then
        return self.createTestMap(level, containsLockedBrick)
    else
        return bricks
    end
end
