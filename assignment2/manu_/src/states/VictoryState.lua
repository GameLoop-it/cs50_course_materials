...
function VictoryState:enter(params)
    ...
    self.containsLockedBrick = ( math.random(100) < 20 )
    self.keyActive = params.keyActive
end
...
function VictoryState:update(dt)
    ...
    if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
        gStateMachine:change('serve', {
            level = self.level + 1,
            bricks = LevelMaker.createMap(self.level + 1, self.containsLockedBrick),
            paddle = self.paddle,
            health = self.health,
            score = self.score,
            ...
            containsLockedBrick = self.containsLockedBrick,
            keyActive = false
        })
    end
end
