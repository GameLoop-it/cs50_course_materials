...
function Room:generateEntities()
    local types = {'skeleton', "skeleton2", 'slime', 'bat', 'ghost', 'spider'}
...
function Room:generateObjects()
    ...
    for i = 1, math.random(5) do
        self:generatePot(self:randomValidX(), self:randomValidY())
    end
end
...
function Room:update(dt)
    ...
    for i = #self.entities, 1, -1 do
        ...
        elseif not entity.dead then
            entity:processAI({room = self, IA = entity.IA}, dt)
            entity:update(dt)
        end
    ...
    for k, object in pairs(self.objects) do
        ...
        if self.player:collides(object) and object.type ~= "projectile" then
            object:onCollide(self.player)
        end
        if object.type == "projectile" and object.bumped == true then
...
function Room:randomValidX()
    return math.random(MAP_RENDER_OFFSET_X + TILE_SIZE,
                    VIRTUAL_WIDTH - TILE_SIZE * 2 - 16)
end

function Room:randomValidY()
